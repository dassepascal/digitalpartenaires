                    ...
                    <div class="flex items-center space-x-4">
                        ...
                        @auth
                            @if ($user->favoritePosts()->exists())
                                <a title="{{ __('Favorites posts') }}" href="{{ route('posts.favorites') }}"><x-icon
                                        name="s-star" class="w-7 h-7" /></a>
                            @endif
                        @endauth
                        <x-menu>
                        ...
                        @if ($user = auth()->user())
                            <x-dropdown>
                                ...
                                <span class="text-black bg-red-500">
                                    @if ($user->isAdmin())
                                        <x-menu-item title="{{ __('Administration') }}"
                                ...
                                <span class="text-black">
                                    @if ($user->isAdmin())
                                        <x-menu-item title="{{ __('Administration') }}"
                                            link="{{ route('admin.dashboard') }}" />
                                    @endif
                                    <x-menu-item title="{{ __('My profile') }}" icon="s-user"
                                        link="{{ route('profile') }}" />
                                    <x-menu-item title="{{ __('My addresses') }}" icon="s-map-pin"
                                        link="{{ route('addresses') }}" />
                                    <x-menu-item title="{{ __('RGPD') }}" icon="s-shield-check"
                                        link="{{ route('rgpd') }}" />
                                    <x-menu-item title="{{ __('Logout') }}" icon="s-arrow-right-on-rectangle"
                                        wire:click="logout" />
                                </span>
                            </x-dropdown>
                        @else
